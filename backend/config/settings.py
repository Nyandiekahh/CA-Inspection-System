...
# ALLOWED_HOSTS = config('ALLOWED_HOSTS', default='localhost,127.0.0.1,0.0.0.0,twvtrpm5-8000.euw.devtunnels.ms').split(',')
...
# # CORS settings - DEV TUNNEL COMPATIBLE VERSION
# # Microsoft dev tunnels automatically handle CORS, so we disable django-cors-headers
# # when using dev tunnels to avoid header conflicts

# # Check if we're using dev tunnels
# USING_DEV_TUNNEL = any('devtunnels.ms' in host for host in ALLOWED_HOSTS if isinstance(host, str))

# if USING_DEV_TUNNEL:
#     # DISABLE django-cors-headers completely when using dev tunnels
#     # The tunnel service handles CORS automatically
#     pass  # No CORS settings needed - tunnel handles it
...
# # Only set CORS headers if NOT using dev tunnels
# if not USING_DEV_TUNNEL:
#     CORS_ALLOW_HEADERS = [
#         'accept',
#         'accept-encoding',
#         'authorization',
...
# For Local Host uncomment the code below header
...
# SMART CORS CONFIGURATION - Auto-detects environment and adapts
# Check if we're using dev tunnels (which handle CORS automatically)
USING_DEV_TUNNEL = any('devtunnels.ms' in host for host in ALLOWED_HOSTS if isinstance(host, str))

# MIDDLEWARE - Conditionally include CORS middleware
MIDDLEWARE = [
    'django.middleware.security.SecurityMiddleware',
    'django.contrib.sessions.middleware.SessionMiddleware',
    'django.middleware.common.CommonMiddleware',
...
# Only add CORS middleware if NOT using dev tunnels
if not USING_DEV_TUNNEL:
...
# SMART CORS SETTINGS - Only apply if NOT using dev tunnels
if not USING_DEV_TUNNEL:
    if DEBUG:
        # Development settings (for localhost testing)
        CORS_ALLOW_ALL_ORIGINS = True
        CORS_ALLOW_CREDENTIALS = True
    else:
        # Production settings - more restrictive
        CORS_ALLOW_CREDENTIALS = True
        CORS_ALLOWED_ORIGINS = [
            'https://ca-inspection-system.vercel.app',  # Your Vercel domain
            'https://your-custom-domain.com',          # Add other production domains here
            'http://localhost:3000',                   # Local development
            'http://127.0.0.1:3000',                  # Local development
        ]
        
        # Optional: Use regex for dynamic subdomains
        CORS_ALLOWED_ORIGIN_REGEXES = [
            r"^http://localhost:\d+$",
            r"^http://127\.0\.0\.1:\d+$",
        ]
...
# Add dev tunnel to CSRF trusted origins if using it
if USING_DEV_TUNNEL:
    dev_tunnel_url = next((host for host in ALLOWED_HOSTS if 'devtunnels.ms' in host), None)
    if dev_tunnel_url:
        CSRF_TRUSTED_ORIGINS.append(f'https://{dev_tunnel_url}')

# Session settings
SESSION_COOKIE_AGE = 86400  # 24 hours
SESSION_COOKIE_HTTPONLY = False  # Allow JS access for API usage
